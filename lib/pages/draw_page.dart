...
class _DrawPageState extends State<DrawPage> {
  ...
  void initState() {
    ...
    controller.addListener(() {
      ...
      if ((newScale - currentScale).abs() > 0.01) setState(() => currentScale = newScale);
    });
    SchedulerBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final size = MediaQuery.of(context).size;
      controller.value = Matrix4.identity()..translate((size.width - canvasWidth)/2, (size.height - canvasHeight)/2)..scale(1.0);
  ...
  void _initLayers() { if (layers.isEmpty) { layers.add(DrawingLayer(id: 'layer_1', strokes: [])); activeLayerIndex = 0; } }
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (mounted) {
        ...
        setState(() { currentName = name; layers[0].strokes = strokes; images..clear()..addAll(loadedImages); texts..clear()..addAll(loadedTexts); isInitialLoading = false; });
      }
    } catch (e) { if (mounted) setState(() => isInitialLoading = false); }
  }
  ...
  void _beginHandleTransform({required TransformHandle handle, required Offset pointerScene}) {
    ...
    if (handle == TransformHandle.rotate) { _transformStartAngle = math.atan2(pointerScene.dy - handles.center.dy, pointerScene.dx - handles.center.dx); _transformRotationStart = _selectedImageIndex != null ? images[_selectedImageIndex!].rotation : texts[_selectedTextIndex!].rotation; }
    else if (handle == TransformHandle.scale) { _transformStartDistance = (pointerScene - handles.center).distance; _transformScaleStart = _selectedImageIndex != null ? images[_selectedImageIndex!].scale : texts[_selectedTextIndex!].scale; }
  }

  void _updateHandleTransform({required Offset pointerScene}) {
    final center = _transformElementCenter; if (_activeHandle == TransformHandle.none || center == null) return;
    if (_activeHandle == TransformHandle.rotate) {
      ...
      setState(() { if (_selectedImageIndex != null) { final img = images[_selectedImageIndex!]; img.scale = newScale; img.position = center - Offset((img.image.width * img.scale) / 2, (img.image.height * img.scale) / 2); } else { final t = texts[_selectedTextIndex!]; t.scale = newScale; final tp = _layoutTextPainter(t); final pad = t.backgroundColor == null ? 0.0 : t.padding * 2; t.position = center - Offset(((tp.width + pad) * t.scale) / 2, ((tp.height + pad) * t.scale) / 2); } });
    }
  }
  ...
  SelectionHandles? _currentSelectionHandles() {
    ...
    if (_selectedImageIndex != null) { final img = images[_selectedImageIndex!]; w = img.image.width * img.scale; h = img.image.height * img.scale; rot = img.rotation; pos = img.position; }
    else { final t = texts[_selectedTextIndex!]; final tp = _layoutTextPainter(t); final pad = t.backgroundColor == null ? 0.0 : t.padding * 2; w = (tp.width + pad) * t.scale; h = (tp.height + pad) * t.scale; rot = t.rotation; pos = t.position; }
  ...
  int? _hitTestImage(Offset p) { for (int i = images.length - 1; i >= 0; i--) { final img = images[i]; if (SelectionHandles.isPointInRotatedRect(point: p, center: img.position + Offset((img.image.width*img.scale)/2, (img.image.height*img.scale)/2), width: img.image.width*img.scale, height: img.image.height*img.scale, rotation: img.rotation)) return i; } return null; }
  int? _hitTestText(Offset p) { for (int i = texts.length - 1; i >= 0; i--) { final t = texts[i]; if (t.text.trim().isEmpty) continue; final tp = _layoutTextPainter(t); final pad = t.backgroundColor == null ? 0.0 : t.padding * 2; final w = (tp.width + pad) * t.scale; final h = (tp.height + pad) * t.scale; if (SelectionHandles.isPointInRotatedRect(point: p, center: t.position + Offset(w/2, h/2), width: w, height: h, rotation: t.rotation)) return i; } return null; }
  ...
  Future<void> _promptAddOrEditText({Offset? position, int? existingIndex}) async {
    ...
    if (result == true && initial.text.trim().isNotEmpty) {
      setState(() {
        if (isEdit) { texts[existingIndex] = initial; }
        else { texts.add(initial); _selectedTextIndex = texts.length - 1; }
